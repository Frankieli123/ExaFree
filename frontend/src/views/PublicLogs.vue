      ...
      <section class="rounded-3xl border border-border bg-card p-6">
        <div class="flex flex-wrap items-start justify-between gap-4">
          <div class="flex items-center gap-3">
            <img :src="logoUrl" alt="ExaFree" class="h-8 w-8 object-contain" />
            <div>
        ...
        >
          <div class="text-xs text-muted-foreground">
            展示最近 <span class="font-semibold text-foreground">{{ limit }}</span> 条请求日志
          </div>
          <a
            v-if="chatUrl"
            :href="chatUrl"
          ...
          <span v-else class="text-xs text-muted-foreground">开始请求</span>
        </div>
...
const statCards = computed(() => [
  ...
  { label: '请求次数', value: totalLogs.value },
...
const eventLabel = (event: PublicLogEvent) => {
  if (event.type === 'start') return '开始请求'
