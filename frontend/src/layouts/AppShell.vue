      ...
      <div v-if="isApiInfoOpen" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/30 px-4">
        <div class="w-full max-w-lg rounded-3xl border border-border bg-card p-6 shadow-xl">
          ...
          <div class="mt-4 space-y-3 text-sm">
            ...
            <div>
              <p class="text-xs text-muted-foreground">搜索主接口</p>
              <div class="mt-1 flex items-start gap-2">
                <p class="min-w-0 flex-1 break-all rounded-2xl border border-border bg-background px-3 py-2 font-mono text-xs">
                  {{ apiSearchUrl }}
                </p>
            ...
            <div>
              <p class="text-xs text-muted-foreground">支持接口</p>
              <div class="mt-1 rounded-2xl border border-border bg-background px-3 py-2 text-xs text-muted-foreground">
                <div class="flex flex-wrap gap-2 text-foreground">
                  <span
                    v-for="apiPath in supportedApis"
                    :key="apiPath"
            ...
            <div>
              <p class="text-xs text-muted-foreground">鉴权说明</p>
              <div class="mt-1 flex items-start gap-2">
...
const adminMenuItems = [
  ...
  },
  {
    path: '/users',
    label: '用户管理',
    icon: 'M16 11a4 4 0 1 0-4-4 4 4 0 0 0 4 4zm-8 1a3 3 0 1 0-3-3 3 3 0 0 0 3 3zm0 2c-2.8 0-5 1.4-5 3.2V20h8v-2.8c0-1.3-.7-2.4-1.8-3.2H8zm8.1 0H16c-1.4 0-2.7.4-3.8 1.1 1 .9 1.8 2.1 1.8 3.7V20h7v-1.2c0-2.6-2.2-4.8-4.9-4.8z',
  },
  {
    path: '/apikeys',
    label: 'APIKey 管理',
    icon: 'M12 3a5 5 0 0 0-5 5v2H6a2 2 0 0 0-2 2v7h16v-7a2 2 0 0 0-2-2h-1V8a5 5 0 0 0-5-5zm-3 7V8a3 3 0 1 1 6 0v2H9zm3 3a2 2 0 0 1 1 3.73V18h-2v-1.27A2 2 0 0 1 12 13z',
  },
  {
    path: '/profile',
    label: '账户设置',
    icon: 'M12 12a4 4 0 1 0-4-4 4 4 0 0 0 4 4zm0 2c-3.9 0-7 2-7 4.5V21h14v-2.5C19 16 15.9 14 12 14z',
  },
  {
    path: '/settings',
    label: '系统设置',
...
const menuItems = computed(() => (authStore.isAdmin ? adminMenuItems : userMenuItems))
...
const apiSearchUrl = computed(() => `${apiBaseUrl.value}/search`)
const apiKeyDisplay = computed(() => {
  if (authStore.isAdmin) {
    return '已移除全局 API_KEY。请在“用户管理 / APIKey 管理”使用用户密钥。'
  }
  return authStore.keys?.[0]?.key_prefix || '请到 APIKey 管理新建'
})
const supportedApis = [
  '/search',
  '/answer',
