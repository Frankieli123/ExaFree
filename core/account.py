...
class AccountManager:
    ...
    async def get_jwt(self, request_id: str = "") -> str:
        """Exa 模式下不再使用 JWT，会话由 Exa API key 直接鉴权。"""
        raise HTTPException(410, "JWT flow is disabled in Exa mode")
...
def load_multi_account_config(
    ...
    for i, acc in enumerate(accounts_data, 1):
        # 兼容 Exa 账号与历史账号配置：
        # - Exa: exa_api_key 必填（旧字段可为空）
        # - 旧结构: secure_c_ses/csesidx/config_id 仍沿用
        is_exa = bool(acc.get("exa_api_key"))
        if not is_exa:
        ...
        config = AccountConfig(
            ...
            exa_api_key=acc.get("exa_api_key"),
            coupon_status=acc.get("coupon_status"),
