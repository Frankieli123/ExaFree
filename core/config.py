...
class BasicConfig(BaseModel):
    ...
    linuxdo_oauth_enabled: bool = Field(default=False, description="是否启用 Linux DO OAuth 登录")
    ...
    linuxdo_authorize_url: str = Field(default="https://connect.linux.do/oauth2/authorize", description="Linux DO OAuth 授权地址")
    linuxdo_token_url: str = Field(default="https://connect.linux.do/oauth2/token", description="Linux DO OAuth Token 地址")
    ...
    exa_redeem_coupon_enabled: bool = Field(default=False, description="注册后是否自动兑换 Exa 优惠码")
    exa_coupon_code: str = Field(default="", description="Exa 优惠码（启用时生效）")
...
class ConfigManager:
    ...
    def load(self):
        ...
        basic_config = BasicConfig(
            ...
            linuxdo_oauth_enabled=_parse_bool(basic_data.get("linuxdo_oauth_enabled"), False),
            linuxdo_client_id=str(basic_data.get("linuxdo_client_id") or "").strip(),
            linuxdo_client_secret=str(basic_data.get("linuxdo_client_secret") or "").strip(),
            linuxdo_authorize_url=str(
                basic_data.get("linuxdo_authorize_url") or "https://connect.linux.do/oauth2/authorize"
            ).strip(),
            linuxdo_token_url=str(
                basic_data.get("linuxdo_token_url") or "https://connect.linux.do/oauth2/token"
            ).strip(),
            linuxdo_userinfo_url=str(
                basic_data.get("linuxdo_userinfo_url") or "https://connect.linux.do/api/user"
            ).strip(),
            linuxdo_redirect_uri=str(basic_data.get("linuxdo_redirect_uri") or "").strip(),
            linuxdo_scope=str(basic_data.get("linuxdo_scope") or "openid profile email").strip(),
            ...
            exa_redeem_coupon_enabled=_parse_bool(exa_redeem_coupon_enabled_raw, False),
            exa_coupon_code=str(exa_coupon_code_raw or "").strip(),
