...
class DuckMailClient:
    ...
    def fetch_verification_code(self, since_time=None) -> Optional[str]:
        ...
        try:
            ...
            def _extract_message_id(msg_obj) -> Optional[str]:
                raw_id = msg_obj.get("id")
                if isinstance(raw_id, str) and raw_id.strip():
                    return raw_id.strip()

                at_id = msg_obj.get("@id")
                if isinstance(at_id, str) and at_id.strip():
                    parts = [p for p in at_id.strip().split("/") if p]
                    if parts:
                        return parts[-1]

                alt_id = msg_obj.get("msgid") or msg_obj.get("messageId")
                if isinstance(alt_id, str) and alt_id.strip():
                    return alt_id.strip()
            ...
            skipped_by_time = 0
            filtered_messages = []
            time_grace = timedelta(minutes=5)
            for idx, msg in enumerate(messages, 1):
                ...
                if since_time:
                    ...
                    # 邮件服务和容器时间可能有秒级偏差，增加容差避免误过滤验证码邮件
                    if msg_time and msg_time < (since_time - time_grace):
                        skipped_by_time += 1
                        continue

                filtered_messages.append((idx, msg_id))

            # 如果时间过滤后全部被跳过，回退到最新几封进行兜底检查
            if not filtered_messages and skipped_by_time:
                self._log("warning", f"⚠️ 时间过滤跳过了 {skipped_by_time} 封邮件，启用兜底检查最新邮件")
                for idx, msg in enumerate(messages[:5], 1):
                    msg_id = _extract_message_id(msg)
                    if msg_id:
                        filtered_messages.append((idx, msg_id))

            for idx, msg_id in filtered_messages:
