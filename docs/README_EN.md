...
<p align="center">Native Exa search API proxy with multi-account load balancing, image generation, video generation, multimodal capabilities, and built-in admin panel.</p>
...
- Violations of Exa's Terms of Service
...
- ✅ Native Exa API proxy - Focused on search and research workflows
...
| ------------------------ | ------ | ---------- | --------------- | --------- | --------- |
| `Exa-auto`            | ✅      | ✅          | ✅               | Optional  | -         |
| `Exa-2.5-flash`       | ✅      | ✅          | ✅               | Optional  | -         |
| `Exa-2.5-pro`         | ✅      | ✅          | ✅               | Optional  | -         |
| `Exa-3-flash-preview` | ✅      | ✅          | ✅               | Optional  | -         |
| `Exa-3-pro-preview`   | ✅      | ✅          | ✅               | Optional  | -         |
| `Exa-3.1-pro-preview` | ✅      | ✅          | ✅               | Optional  | -         |
| `Exa-imagen`          | ✅      | ✅          | ✅               | ✅         | -         |
| `Exa-veo`             | ✅      | ✅          | ✅               | -         | ✅         |

> `Exa-imagen`: Dedicated image generation model · `Exa-veo`: Dedicated video generation model
...
python main.py
# Background with pm2
pm2 start main.py --name Exa-api --interpreter ./.venv/bin/python3
```
...
python main.py
# Background with pm2
pm2 start main.py --name Exa-api --interpreter ./.venv/Scripts/python.exe
```
...
- **API Endpoint**: `http://localhost:7860/search`
...
| `/search` | POST | Search |
| `/answer` | POST | Answer |
| `/contents` | POST | Content fetch |
| `/findSimilar` | POST | Similar content lookup |
| `/research/v1` | POST/GET | Research task create/query |
...
> Authentication is user-based only. Create/register a user first, then call APIs with that user's API Key.
...
> Docker image: `cooooookk/exafree:latest`
>
...
To deploy the account refresh service separately from the main API, use the [`refresh-worker` branch](https://github.com/Dreamy-rain/exafree/tree/refresh-worker):

```bash
git clone -b refresh-worker https://github.com/Dreamy-rain/exafree.git Exa-refresh-worker
cd Exa-refresh-worker
cp .env.example .env
...
[![Star History Chart](https://api.star-history.com/svg?repos=Dreamy-rain/exafree&type=date&legend=top-left)](https://www.star-history.com/#Dreamy-rain/exafree&type=date&legend=top-left)
