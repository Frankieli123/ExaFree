...
- 📝 **Exa Workspace** - 3 种格式（Docs, Sheets, Slides）
...
| 格式          | 扩展名     | MIME 类型                                  | 支持状态 | 说明         |
| ------------- | ---------- | ------------------------------------------ | -------- | ------------ |
| Exa Docs   | `.gdoc`    | `application/vnd.Exa-apps.document`     | ✅ 支持   | 需要导出链接 |
| Exa Sheets | `.gsheet`  | `application/vnd.Exa-apps.spreadsheet`  | ✅ 支持   | 需要导出链接 |
| Exa Slides | `.gslides` | `application/vnd.Exa-apps.presentation` | ✅ 支持   | 需要导出链接 |
...
```bash
curl -X POST http://localhost:7860/contents \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer your_api_key" \
  -d '{
...
```bash
curl -X POST http://localhost:7860/findSimilar \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer your_api_key" \
  -d '{
...
- [项目主页](https://github.com/Dreamy-rain/exafree)
- [API 文档](README.md)
- [问题反馈](https://github.com/Dreamy-rain/exafree/issues)
